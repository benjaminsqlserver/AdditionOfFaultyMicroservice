namespace StudentManagement.Extensions.Services;

using StudentManagement.Databases;
using StudentManagement.Resources;
using StudentManagement.Services;
using StudentManagement.Resources.HangfireUtilities;
...
public static class ServiceRegistration
{
    public static void AddInfrastructure(this IServiceCollection services, IWebHostEnvironment env, IConfiguration configuration)
    {
        ...
        var connectionString = configuration.GetConnectionStringOptions().StudentManagement;
        if(string.IsNullOrWhiteSpace(connectionString))
        {
            // this makes local migrations easier to manage. feel free to refactor if desired.
            connectionString = env.IsDevelopment() 
                ? "Data Source=localhost,52396;Integrated Security=False;Database=dev_studentmanagement;User ID=SA;Password=#localDockerPassword#"
        ...
        services.AddDbContext<StudentManagementDbContext>(options =>
            options.UseSqlServer(connectionString,
                builder => builder.MigrationsAssembly(typeof(StudentManagementDbContext).Assembly.FullName))
                            .UseSnakeCaseNamingConvention());

        services.AddHostedService<MigrationHostedService<StudentManagementDbContext>>();
