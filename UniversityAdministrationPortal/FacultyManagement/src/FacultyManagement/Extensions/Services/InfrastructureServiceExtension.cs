namespace FacultyManagement.Extensions.Services;

using FacultyManagement.Databases;
using FacultyManagement.Resources;
using FacultyManagement.Services;
using FacultyManagement.Resources.HangfireUtilities;
...
public static class ServiceRegistration
{
    public static void AddInfrastructure(this IServiceCollection services, IWebHostEnvironment env, IConfiguration configuration)
    {
        ...
        var connectionString = configuration.GetConnectionStringOptions().FacultyManagement;
        if(string.IsNullOrWhiteSpace(connectionString))
        {
            // this makes local migrations easier to manage. feel free to refactor if desired.
            connectionString = env.IsDevelopment() 
                ? "Data Source=localhost,55729;Integrated Security=False;Database=dev_facultymanagement;User ID=SA;Password=#localDockerPassword#"
                : throw new Exception("The database connection string is not set.");
        }

        services.AddDbContext<FacultyManagementDbContext>(options =>
            options.UseSqlServer(connectionString,
                builder => builder.MigrationsAssembly(typeof(FacultyManagementDbContext).Assembly.FullName))
                            .UseSnakeCaseNamingConvention());

        services.AddHostedService<MigrationHostedService<FacultyManagementDbContext>>();
